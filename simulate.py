...
def print2d_list(twodlist):

	for i in range(len(twodlist)):
		row_valu=[]
		for j in range(len(twodlist[i])):
			row_valu.append(str(twodlist[i][j])[0:6])
...
class Simulation(object):
	...
	def move(self, dy, dx):
		...
		print("prev_pose (y {},x {})".format(self.true_pose[0],self.true_pose[1]))
		new_y = (self.true_pose[0] + dy) % self.height
		new_x = (self.true_pose[1] + dx) % self.width
		self.prev_pose = self.true_pose
		self.true_pose = (new_y, new_x)
		print("prev_pose (new_y {},new_x {})".format(self.true_pose[0],self.true_pose[1]))
